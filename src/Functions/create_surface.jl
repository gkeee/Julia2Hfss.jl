module create_surface 
    export Csurface
    using LaTeXStrings  
    function Csurface(ProjectName::String, PackagePath::String, LineName::String, X::Vector, Y::Vector)
    
        PointsNumber = length(X);
        file = open("$(PackagePath)\\Julia2Hfss.jl\\src\\Functions\\create_surface.vbs", "w")
        ...
        write(file, "  \"SolveInside:=\", true,_\n")
        write(file, "  \"ShellElement:=\", false, \"ShellElementThickness:=\", \"0mm\", _\n")
        write(file, "  \"ReferenceTemperature:=\", \"20cel\", \"IsMaterialEditable:=\", _\n")
